...
class Manager:
    ...
    def phase1Execute():
        ...
        print(Fore.YELLOW + "Phase 1: Manager is executing the query" + Fore.RESET)
        isManagerReady = True
        try:
        ...
        askOperator = input(
            Fore.WHITE + "Is the operator ready to commit? (y/n): " + Fore.RESET).lower()
        ...
        if isManagerReady and isOperatorReady:
            print(Fore.GREEN + "Manager is ready to execute the query" + Fore.RESET)
            Manager.log_file.write(
                "Ready===" + transactionID + "===" + query + "\n")
            Manager.log_file.flush()
            return jsonify({"status": "READY "+query})
        else:
            print(Fore.RED + "Manager is not ready to execute the query" + Fore.RESET)
            Manager.log_file.write(
                "No===" + transactionID + "===" + query + "\n")
            Manager.log_file.flush()
    ...
    def phase2Execute():
        ...
        print(Fore.YELLOW + "Phase 2: Manager is executing the query" + Fore.RESET)
        print("Coordinator decision: " + coordinator_decision)
        if coordinator_decision == "COMMIT":
            print(Fore.GREEN + "Committing the transaction" + Fore.RESET)
            Manager.log_file.write(
                "Commit===" + transactionID + "===" + query + "\n")
            Manager.log_file.flush()
            Manager.connection.commit()
            print("Transaction committed by " + Manager.node_id + " manager")
            return jsonify({"status": "COMMIT "+query})
        else:
            print(Fore.RED + "Aborting the transaction" + Fore.RESET)
            Manager.log_file.write(
                "Abort===" + transactionID + "===" + query + "\n")
            Manager.log_file.flush()
    ...
    def failureRecovery(self):
        ...
        with open(self.log_file.name, "r") as f:
            lines = f.readlines()
            if len(lines) == 0 or lines[-1] == "Transaction is already committed\n" or lines[-1] == "Transaction is already aborted\n":
            ...
            print(Fore.YELLOW + "Last transactionID: " +
                  Fore.RESET + last_transactionID)
            ...
            if last_decision == "Ready":
                ...
                try:
                    ...
                    if decisionByCoordinator == "COMMIT":
                        ...
                        self.log_file.write(
                            "Commit===" + last_transactionID + "===" + last_query + "\n")
                        self.log_file.flush()
                        self.connection.commit()
                        print("Transaction committed by " +
                              self.node_id + " manager")
                    else:
                        print("Aborting the transaction")
                        self.log_file.write(
                            "Abort===" + last_transactionID + "===" + last_query + "\n")
                        self.log_file.flush()
                        self.connection.rollback()
                        print("Transaction aborted by " +
                              self.node_id + " manager")
                except:
                    print("Coordinator is not available")
            elif last_decision == "Commit":
                print("Transaction is already committed")
                self.log_file.write("Transaction is already committed\n")
                self.log_file.flush()
            elif last_decision == "Abort":
                print("Transaction is already aborted")
                self.log_file.write("Transaction is already aborted\n")
                self.log_file.flush()
            else:
                print("No transaction to recover")
